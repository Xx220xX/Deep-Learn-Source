...
__kernel void gab_sub(__global double *aL, __global double *y, __global double *dzL, __global double *bL, double hit) {
    int i = get_global_id(0);
    dzL[i] = aL[i] - y[i];
    bL[i] = bL[i] - hit * dzL[i];
}

__kernel void gab_mult_w_a(__global double *dzl, __global double *al_down, int ma_down, __global double *dwl) {
    int i = get_global_id(0);
...
__kernel void gab_update_w(__global double *wl, __global double *dwl, double hit) {
    int i = get_global_id(0);
    wl[i] = wl[i] - hit * dwl[i];
}
